...
def start_background_job(mode, area, label=None, start_date=None, end_date=None):
    job_id = create_job(mode=mode, area=area, label=label, start_date=start_date, end_date=end_date)
...
def run_fetch_job(job_id):
    global tasks, label_to_match, last_successful_filter
...
def start_fetch():
    ...
    if mode == 'dates':
        ...
        job_id = start_background_job(mode='dates', area=selected_area, start_date=start_date, end_date=end_date)
    else:
        label = (request.form.get('query_label') or '').strip()
        if not label:
            return jsonify({'ok': False, 'error': 'Нужно указать метку'}), 400

        job_id = start_background_job(mode='label', area=selected_area, label=label)

    return jsonify({'ok': True, 'job_id': job_id})


@app.route('/refresh-kanban', methods=['POST'])
def refresh_kanban():
    if not last_successful_filter:
        return jsonify({'ok': False, 'error': 'Нет сохраненного фильтра для обновления. Загрузите задачи через форму.'}), 400

    job_id = start_background_job(
        mode=last_successful_filter['mode'],
        area=last_successful_filter['area'],
        label=last_successful_filter.get('label'),
        start_date=last_successful_filter.get('start_date'),
        end_date=last_successful_filter.get('end_date'),
    )
...
def fetch_tasks():
    global tasks, label_to_match, selected_area, last_successful_filter
